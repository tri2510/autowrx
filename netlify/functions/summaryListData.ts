...
export const handler: Handler = async function (event, context) {
    try {
        ...
        if (!resData.empty) {
            resData.forEach((item) => {
                switch (item.data().type) {
                    case "visit":
                        retData.visits.push({ id: item, created_time: item.data().created_time.toDate() });

                        break;
                    case "run-prototype":
                        retData.runs.push({ id: item.id, created_time: item.data().created_time.toDate() });
                        break;
                    default:
                        break;
                }
        ...
        console.log("list Summary Data err", error);
        return {
            statusCode: 400,
            body: error.toString(),
        };
