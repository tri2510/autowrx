...
const V2CApiListItem = ({
  ...
  return (
    <div
      className={`w-full border-b border-border justify-between py-2 px-2 cursor-pointer hover:bg-primary/10 items-center ${isSelected ? 'bg-primary/10' : ''}`}
      onClick={onClick}
    >
      <div className="flex flex-1 truncate items-center">
        <span
          className={`text-xs font-medium px-2 py-1 rounded mr-2 ${getMethodColor(api.method)}`}
        >
          {api.method}
        </span>
        <div className="grow truncate">
          <div
            className={`text-sm ${isSelected ? 'font-medium' : 'font-normal'} truncate`}
          >
            {api.path}
          </div>
        ...
        { api.info_url && <div>
          <a href={api.info_url} target="_blank" rel="noreferrer">
    ...
    <div className="w-full h-full overflow-auto p-4">
      ...
      <div className="mb-4">
        ...
        { api.path && <div className="flex items-center mb-2">
          <span
            className={`text-sm font-medium px-2 py-1 rounded mr-2 ${
              api.method === 'GET'
            ...
            ([code, response]: [string, any]) => (
              <div key={code} className="border rounded p-2">
                <div className="flex items-center mb-1">
                  ...
                  <div className="text-sm font-medium">{response.description}</div>
                </div>
