    ...
    return (
        <div className="flex flex-col w-full overflow-y-auto">
            {reviews.slice(0, visibleReviewsCount).map((review) => (
                <div key={review.id} className="flex flex-col p-3 my-2 bg-da-gray-light rounded">
                    <div className="flex flex-col mb-2 w-full">
                        <div className="flex w-full items-center mb-3">
                            <img src="/imgs/profile.png" alt="profile" className="flex w-5 h-5 rounded-full" />
                            <div className="ml-2 text-sm text-da-gray-medium">{review.createdBy.fullName}</div>
                        </div>
                        <div className="flex items-center">
                            <Rating rating={review.rating} />
                            <div className="da-label-small text-da-gray-dark ml-2">{formatDate(review.createdAt)}</div>
                        </div>
                    </div>
