...
const FormSignIn = ({ setAuthType }: FormSignInProps) => {
  ...
  const signIn = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setLoading(true)
      const [email, password] = [e.currentTarget.email.value, e.currentTarget.password.value]
  ...
  return (
    <form onSubmit={signIn} className='w-[400px] min-w-[400px] min-h-[300px] block px-2 md:px-6 py-2 bg-da-white'>
      {/* Title */}
      ...
      {/* Content */}
      <DaInput name='email' placeholder='Email' label='Email' className='mt-4' Icon={TbAt} iconBefore iconSize={18} />
      <DaInput
        name='password'
      ...
      <div className='flex items-center justify-end mt-1'>
        <DaButton type='button' variant='link' onClick={() => setAuthType("forgot")}>
          Forget Password
        </DaButton>
      </div>
