...
const FormRegister = ({ setAuthType }: FormRegisterProps) => {
  ...
  const validate = (name: string, email: string, password: string, confirmPassword: string) => {
    ...
    if (password !== confirmPassword) errors.push('"password" and "confirm password" must be the same')
    return errors
  }
  ...
  return (
    <form onSubmit={register} className='w-[400px] min-w-[400px] min-h-[300px] block px-2 md:px-6 py-2 bg-da-white'>
      {/* Title */}
      ...
      {/* Content */}
      <DaInput name='fullName' placeholder='Name' label='Name' className='mt-4' />
      ...
      {/* Action */}
      <DaButton disabled={loading} type='submit' variant='gradient' className='w-full mt-6'>
        {loading && <TbLoader className='animate-spin text-lg mr-2' />}
        Register
      </DaButton>
      {/* More */}
      <div className='mt-4 flex items-center'>
        <DaText className='text-da-gray-dark'>Already have an account?</DaText>
        <DaButton type='button' onClick={() => setAuthType("sign-in")} variant='text' className='text-da-primary-500'>
          Sign in
        </DaButton>
      </div>
    </form>
