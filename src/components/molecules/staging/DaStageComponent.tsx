        ...
        { !item.isTopMost && <div className={cn("h-[32px] w-full border-b flex", className)}>
            <div className={`h-full grow flex items-center`}>
                ...
                {(item.children && item.children.length>0)?<>
                    {!isEpanded && <IoIosArrowForward className="cursor-pointer mr-2" size={20} onClick={() => { setIsExpanded(true) }}/> }
            ...
            { !onTargetMode && <div className="h-full px-2 flex items-center justify-center w-24 border-l">
                { item.version && <>
                    { !inEditMode && <CiEdit size={26} className="cursor-pointer hover:opacity-50"
                        onClick={() => {
                            setInEditMode(true)
                        }}/>
                    }
                    { inEditMode && <>
                        <MdOutlineCancel  size={24} className="cursor-pointer hover:opacity-50"
                             onClick={() => { setInEditMode(false) }}/>
                        <IoSaveOutline size={24} className={cn("ml-4 cursor-pointer hover:opacity-50", editVersion.trim().length<0&&'text-da-gray-light')}
                            onClick={() => {
                                if(editVersion.trim().length<0) {
                                    return
                                }
            ...
            { onTargetMode && <>
                <div className="h-full px-2 flex items-center justify-center w-24 border-l">
                    { isTargetConnected &&  item.version && targetState && targetState[item.id] && (item.version!==targetState[item.id]) &&<>
                            { isUpdating && activeId==item.id && <div>Updating...</div>}
                            { !isUpdating && <button className="rounded px-2 py-0 da-btn-solid"
                                onClick={() => {
                                    if(onRequestUpdate) {
                ...
                <div className="h-full px-2 flex items-center justify-center w-24 border-l">
                    { isTargetConnected && <>
                        { item.version && targetState && item.version != targetState[item.id] && 
                        <span className="font-bold text-red-500">{targetState[item.id]}</span>}
                        { item.version && targetState && (!targetState[item.id] || item.version==targetState[item.id]) && 
