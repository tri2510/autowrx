...
const PageHome = () => {
  const instance = import.meta.env.VITE_DEPLOY_INSTANCE
    ? import.meta.env.VITE_DEPLOY_INSTANCE
    : "playground";
  return (
    <div className="grid grid-cols-12 bg-white">
      {(instance === "playground" || instance === "covesa") &&
        instance !== "etas" && (
          <div className="flex col-span-12 relative min-h-[400px] max-h-[400px] w-full justify-between  scale z-10 overflow-hidden">
            <div
              className={classNames(
                ...
                // instance === "covesa" &&
                //   "from-da-gradient-from from-0% via-brand-primary via-60% to-da-gradient-to to-100%"
              )}
            ></div>
              ...
              <div className="lg:w-[50%] lg:px-24 px-12 z-30">
                <div className="flex flex-col sm:text-xs">
                  <div className="flex relative text-2xl lg:text-4xl  font text-white font-bold">
                    Welcome to {instance === "covesa" ? "covesa." : ""}
                    digital.auto playground for virtual exploration
                  </div>
                  <div className="flex text-white pt-2 text-sm lg:text-normal">
                    To support shift-level testing for software-defined vehicle
                    (SDV) applications, we have created the digital.auto
                    playground. This is a cloud-based, rapid prototyping
                    environment for new, SDV-enabled features. The prototypes
                    are built against real-world vehicle APIs and can be
                    seamlessly migrated to automotive runtimes, such as Eclipse
                    Velocitas. The playground is open and free to use.
                  </div>
                </div>
              </div>
            </div>
