...
import { addLog, saveModelSkeleton, savePrototypeSkeleton } from "../../apis";
...
const PrototypeSkeletonPage = () => {
    ...
    const callSave = async (skele: any) => {
        if (!prototype) return;
        try {
            await savePrototypeSkeleton(prototype.id, JSON.stringify(skele));
        } catch (err) {
    ...
    const createNewNode = async () => {
        ...
        if (profile) {
            ...
            addLog(
                ...
                `User '${username}' create new node with id '${id}' in architecture of prototype '${prototype.name}' with id '${prototype.id}'`,
                "create",
                profile.uid,
                null,
                String(id),
                                    ...
                                    <div
                                        ...
                                        onClick={(event) => {
                                            ...
                                            if (event.stopPropagation) {
                                                event.stopPropagation(); // W3C prototype
                                            }
                                        }}
                                    >
                                        Copy Link
                                            ...
                                            if (confirm(`Do you confirm to delete node '${node.name}'?`)) {
                                                ...
                                                if (profile) {
                                                    ...
                                                    addLog(
                                                        ...
                                                        `User '${username}' delete node with id '${node.id}' in architecture of prototype '${prototype.name}' with id '${prototype.id}'`,
                                                        "delete",
                                                        profile.uid,
                                                        null,
                                                        String(node.id),
...
export default PrototypeSkeletonPage;
