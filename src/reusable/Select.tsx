...
const Select = ({prefix = "", options, containerClassName, state, autoSelectFirst = true, disabled, onSelect, onClick}: SelectProps) => {
    ...
    return (
        <div className={clsx("flex w-fit relative items-center", !disabled && "cursor-pointer", containerClassName)} onClick={onClick}>
            <select
            ...
            <div className={clsx("flex", disabled && "opacity-40 cursor-default")}>
                <div className="flex whitespace-nowrap">{prefix}{selected?.name ?? ""}</div>
