...
export const loadMarketWidgets = async () => {
    try {
        let res = await axios.get(`${config.widgetMarketPlaceBe}/package?category=widget&limit=50&page=1`);
...
export const searchWidget = async (widgetId: any) => {
    try {
        let res = await axios.get(`${config.widgetMarketPlaceBe}/package/${widgetId}`);
...
export const loadWidgetReviews = async (widgetId:any) => {
    try {
        let res = await axios.get(`${config.widgetMarketPlaceBe}/review?packageId=${widgetId}&limit=20&offset=0`);
