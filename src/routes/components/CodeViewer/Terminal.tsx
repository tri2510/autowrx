...
const Terminal = ({title = "Terminal", openClassName = "h-96", codeClassName = "", open, setOpen, lines, hideMinimize = false, outerClassName}: TerminalProps) => {
                                ...
                                <div className="mt-2 flex font-mono last:pb-4" key={random()}>
                                    ...
                                    <p className={clsx("flex-1 items-center pl-2 whitespace-pre-line")}>
                                        {line.parts.map((part, i) => {
                                            return <span key={i + "-" + part.type + " - " + part.text} className={part.type === "error" ? "text-red-400" : ""}>{part.text}</span>
                                        })}
                                    </p>
                                </div> 
                            ))}
