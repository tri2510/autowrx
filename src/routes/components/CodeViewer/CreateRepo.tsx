    ...
    return repoCreatedError === null ? (
        <div className="flex flex-col">
            ...
            <a
                ...
                target="_blank"
                rel="noreferrer"
                className="bg-gray-100 p-2 px-3 rounded-md transition hover:bg-gray-200"
            >
                {repoCreatedUrl}
            </a>
        </div>
...
const RepoCreationForm: FC<RepoCreationFormProps> = ({
    code,
    model,
    user,
    accessToken,
    ...
    const [activeOrg, setActiveOrg] = useState(user && user.orgs ? user.orgs[0] : "");
    const [loadingCreation, setLoadingCreation] = useState(false);

    return (
        <div className="flex flex-col">
            <div className="flex text-xl mb-5 select-none text-purple-700 font-bold">
                <VscGithubInverted size="1.4em" className="mr-3" />
                Create Repository
            </div>
            <div className="flex h-full h-16 select-none">
                <div className="flex flex-col h-full mr-3">
                    <div className="text-sm mb-1">Owner</div>
                    <div className="flex items-center font-bold select-none my-auto">
                        <select
                            value={activeOrg}
                            onChange={(e) => setActiveOrg(e.target.value)}
                            className="flex outline-none bg-gray-100 border border-gray-200 rounded-md px-2 h-[34px] mr-3"
                        >
                            {user.orgs &&
                                user.orgs.map((org) => (
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="flex my-auto">
                        <TransparentInput
                            state={[repo, setRepo]}
                            className="flex outline-none bg-gray-100 border border-gray-200 rounded-md py-1 px-3"
                        />
                    </div>
                </div>
            </div>
                    ...
                    onClick={async () => {
                        ...
                        try {
                            await publishToGithub({
                                ...
                                user_login: activeOrg, // user.login,
                                code: code,
                                repo: repo,
                            ...
                            setRepoCreatedUrl(`https://github.com/${activeOrg}/${repo}`);
                        } catch (error) {
                            console.error(error);
...
const CreateRepo: FC<CreateRepoProps> = ({ code, model }) => {
    ...
    return repoCreatedUrl !== null ? (
        <RepoCreatedDisplay repoCreatedError={repoCreatedError} repoCreatedUrl={repoCreatedUrl} />
