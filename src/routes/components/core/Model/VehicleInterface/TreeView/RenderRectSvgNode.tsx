        ...
        <g>
            <g
                onClick={() => {
                    ...
                    const fullLink = prototype_id === "" ? `/model/${model.id}${cviLink}/` : `/model/${model.id}/library/prototype/${prototype_id}/view${cviLink}/`
            ...
            >
                {(
                    <>
                        <rect
                        width={nodeWidth} height={40} y={-20} x={-50} rx={10} strokeWidth="0"
            ...
            {
                node.type === "branch" && !collapsed && (<circle
                    cx={-50}
                    cy={20}
                    ...
                    stroke={"white"}
                    onClick={() => {
                        const cviLink = node.path === "" ? `/cvi/list/${node.name}` : `/cvi/list/${node.path}.${node.name}`
                        const fullLink = prototype_id === "" ? `/model/${model.id}${cviLink}/` : `/model/${model.id}/library/prototype/${prototype_id}/view${cviLink}/`
