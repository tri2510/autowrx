        ...
        <div className="flex flex-col h-full">
            ...
            {States.Type[0] !== "branch" && (
                <div className="flex flex-col mb-3">
                    ...
                    <Select options={dataTypes.map(dataType => ({
                        name: dataType,
                        value: dataType
            ...
            <div className="flex mt-auto">
                ...
                onClick={async () => {
                    if (editingNode === null) {
                        ...
                        if (allApis.includes(full_name)) {
                            TriggerPopup(
                                <span className="text-xl"><strong>{full_name}</strong> already exists.</span>,
                                "!w-fit !min-w-fit"
                            )
                            return
                        }
                        ...
                        if (!branchApis.includes(nesting)) {
                            TriggerPopup(
                                <span className="text-xl"><strong>{nesting}</strong> is not a valid branch.</span>,
                                "!w-fit !min-w-fit"
                            )
                            return
                        }
                ...
                >{editingNode ? "Save" : "Create"}</Button>
            </div>
        </div>
    )
}
