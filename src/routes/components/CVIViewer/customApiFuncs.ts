import { deleteField, doc, DocumentData, FieldPath, getDocs, query, Query, runTransaction, updateDoc, where } from "firebase/firestore"
import { getModel } from "../../../apis"
import { db, REFS } from "../../../apis/firebase"
import { DataType, LeafTypes } from "../core/Model/VehicleInterface/Spec"
...
export const createApi: createApiFuncType = async (props) => {
    ...
    await updateDoc(
        doc(REFS.model, props.model_id),
        new FieldPath("custom_apis", nesting, name),
        updateFields
    )
...
export const deleteApi = async (model_id: string, node_name: string) => {
    ...
    const mainKeysToDelete = Object.keys(custom_apis).filter((key) => key === node_name || key.startsWith(`${node_name}.`))

    await runTransaction(db, async (tx) => {
        tx.update(
            doc(REFS.model, model_id),
            new FieldPath("custom_apis", nesting, name),
            deleteField()    
        )
