...
const BranchViewer = ({
    ...
    const fullLink = prototype_id === "" ? `/model/:model_id${cviLink}/` : `/model/:model_id/library/prototype/${prototype_id}/view${cviLink}/`
    ...
    const rowJsx = (
        ...
        >
            ...
            {isCustom && <div className="flex m-auto items-center justify-center bg-fuchsia-500 text-white rounded-xl w-4 h-4 ml-2 font-bold p-2" style={{fontSize: "0.6rem", lineHeight: 0}}>W</div>}
            <span
            className={clsx(
            ...
            <HiChevronRight className="ml-auto h-full my-auto text-gray-300 hidden" style={{transform: "scale(1.2)"}}/>
        </div>
    )

    return (
        <Fragment>
            {(typeof filter === "undefined" || filter(prefix + name)) && (typeof onClickApi === "undefined" ? (
                <LinkWrap to={fullLink}>
                    {rowJsx}
                </LinkWrap>
            ) : (
                <div onClick={() => onClickApi(prefix + name)}>
                    {rowJsx}
                </div>
            ))}
            {node.type === "branch" && (
                Object.entries(model.custom_apis?.[`${prefix}${name}`] ?? {}).map(([childName, subNode]) => (
                    <BranchViewer
