...
export const flattenApis = (
    ...
    for (const [name, node] of Object.entries(cvi)) {
        ...
        list.push(name);
        if (node.type === "branch") {
            list.push(...flattenApis(node.children, filter).map((subname) => `${name}.${subname}`));
        }
    }
    return list;
};
...
export const getFlattenedApis = (model: Model, filter?: (node: AnyNode) => boolean) => {
    const builtInApis = flattenApis(
        JSON.parse(model.cvi) as {
            [key: string]: Branch;
        },
        filter
    );
    const customApis = Object.entries(model.custom_apis ?? {})
        .map(([nesting, children]) => {
