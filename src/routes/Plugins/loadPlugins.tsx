...
import { InterceptorType, PluginFunction, PluginPropPrototype, PluginPropSimulator, PluginPropWidgets, SimulatorModifier } from "./pluginTypes";
...
const loadPlugins = (
    ...
    ${plugin_urls.map(({name, url}, i) => dedent(`
        try {
    ...
    const vehicle = createModelProxy({...vehicleObjectOpts, interceptor}, "Vehicle")
    const modelObjectCreator = (root_name: string) => createModelProxy({...vehicleObjectOpts, interceptor}, root_name)
    ...
    ;(window as any)[id] = (pluginsFuncs: [string, PluginFunction | null][]) => {
        for (const [name, pluginFuncs] of pluginsFuncs) {
            ...
            try {
                const hooks = pluginFuncs({widgets: widgetsCreator(name), simulator, prototype, vehicle, modelObjectCreator}) || {}
                pluginHooks[name] = Object.fromEntries(Object.entries(hooks).map(([hookName, hookFunc]) => [
                    hookName,
