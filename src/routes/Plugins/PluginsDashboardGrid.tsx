        ...
        <SquareGrid columns={5} fixedWidth="calc(100vw - 40px)">
            {new Array(10).fill(true).map((_, i) => {
                ...
                return (
                    <div
                        key={"no-exist" + space}
                        className={clsx("flex w-full h-full", getGridItemSpan(gridItem))}
                        data-space={space}
                    >
                        {typeof widget === "undefined" ? (
                            <div className="flex w-full h-full justify-center items-center text-center text-gray-500 break-words select-none p-3">
                                <span className="overflow-hidden">
                                    <strong>{gridItem.plugin + "::" + gridItem.widget}</strong> doesn't exist.
                                </span>
                            </div>
                        ) : (
                            <InjectableIframe key={space} gridItem={gridItem} widget={widget} />
                        )}
                    </div>
                );
            })}
