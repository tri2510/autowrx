...
const HTMLNodeElement: FC<{
    ...
    return (
        <div className="flex w-full h-full" ref={ref}></div>
    )
}
...
const InjectableIframeUnmemoized: FC<InjectableIframeProps> = ({gridItem, widget}) => {
    ...
    useEffect(() => {
        try {
            ...
            alert(`${widget.plugin_name + "::" + widget.name}'s onActivate function threw an error. Check console for the error.`)
            console.error(error)
        }
    }, [ref.current])
