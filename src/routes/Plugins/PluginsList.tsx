        ...
        <div className="flex w-full" style={{ height: "calc(100% - 42px)" }}>
            <div className="flex flex-col h-full w-2/6 border-r">
                <div className="flex w-full">
                    ...
                    {/* { modelPermissions.canEdit() && (   
                        <EditPlugin plugins={plugins} plugin={activePlugin} state={editPluginState}/>
                    )} */}
                    {/* {isLoggedIn && (
                        <div
                ...
                <div className="flex flex-col overflow-auto">
                    {plugins.map((plugin) => {
                        return <PluginOverview plugin={plugin} key={plugin.id} active={plugin.id === plugin_id} />;
                    })}
                </div>
                {plugins.length === 0 && (
                    <div className="flex p-4 text-gray-400 text-2xl w-full h-full justify-center items-center select-none mb-40">
            ...
            <div className="flex flex-col h-full w-4/6 overflow-auto">
                {plugins.length === 0 ? (
                    ...
                    <PluginDisplay plugin={selectedPlugin} plugins={plugins} />
                )}
            </div>
        </div>
    );
