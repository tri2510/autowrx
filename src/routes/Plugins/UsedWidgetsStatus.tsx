...
const UsedWidgetsStatus: FC<{
    ...
    return (
        <div className="absolute right-2" style={{top: "-37px"}}>
            <Dropdown
            onHover
            trigger={
                <div
                className="flex text-gray-500 bg-white z-10 rounded px-3 py-1 select-none items-center cursor-pointer"
            ...
            >
                <div className="flex flex-col w-full py-2">
                    {plugins.map(plugin => (
                        <div className="flex flex-col px-3 py-1 text-sm">
                            ...
                            <div className="flex flex-col pl-3">
                                ...
                                .map(([plugin_widget, widget]) => (
                                    <div className="flex text-gray-500 items-center select-none pt-1">
                                        <VscCircleFilled className="mr-1" color={grid[plugin_widget] ? "#16a34a" : "#9ca3af"} />
                                        <div className="text-sm font-bold cursor-pointer" onClick={() => copyText(widget.name, "Copied widget name!")} >{widget.name}</div>
                                    </div>
                                ))
                                }
                            </div>
