...
const PluginOverview = ({plugin, active}: PluginOverviewProps) => {
    ...
    return (
        <LinkWrap to={`/model/:model_id/plugins/plugin/${plugin.id}`}>
            <div className={clsx("w-full transition py-4 border-b", active && "bg-aiot-blue text-white border-transparent")}>
                ...
                {prototype !== null && (
                    <div className={clsx("text-sm px-4 pt-2", active && "border-transparent")}><strong>Prototype: </strong>{prototype?.name}</div>
                )}
            </div>
        </LinkWrap>
    )
