...
This document contains the complete AosCloud configuration for the digital-auto deployment pipeline.
...
| **Service Provider** | SP nhan.luongnguyen@vn.bosch.com |
...
docker run --rm -v $(pwd):/workspace tri2510/aos-edge-toolchain \
...
### List Services
```bash
docker run --rm --entrypoint "" tri2510/aos-edge-toolchain \
...
```bash
sshpass -p pi123 ssh pi@10.189.232.231 \
